...
class Person(db.Model):
    ...
    def __init__(self, person_info):
        ...
        self.first_name = person_info['name']['first']
    ...
    def __repr__(self):
        return 'gender : {gender}, first name : {fname}, last name : {lname}, email : {email}'.format(
            gender=self.gender, fname=self.first_name, lname=self.last_name, email=self.email)

    def get_full_name(self):
        return '{fname} {lname}'.format(fname=self.first_name, lname=self.last_name).title()
